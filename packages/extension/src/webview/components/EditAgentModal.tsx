...
export interface EditAgentDraft {
  ...
  description: string;
  model: string;
  /** Which CLI/LLM runs the agent (default | codex | opencode). Persisted to
   *  workspace.yaml's AIDLC layer; `default` clears the field. */
  runner: string;
  capabilities: string[];
...
export function EditAgentModal({ agent, skills, onSubmit, onClose }: Props) {
  ...
  const [runner, setRunner] = useState(agent.runner ?? 'default');
  ...
  // Switching runner keeps model choices coherent: a Claude model id is
  // meaningless to codex/opencode, so drop it when leaving `default`; restore
  // a sensible Claude default when returning.
